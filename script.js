...
// Helper to get a color from CSS variable
...
class ThreeCity {
  constructor(container) {
    ...
    this.controls.maxPolarAngle = Math.PI / 2.5; // prevent going underground
    ...
    // Road markings (using small white boxes)
...
// Clean up when leaving city screen (optional)
$('backFromCity')?.addEventListener('click', () => {
  showScreen(mainAppScreen);
  // Keep the city alive so it reloads quickly next time
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Removed: let virtualCity = null; (replaced by threeCity)
  // SCREEN SWITCH
  ...
  // NOTE: The old VirtualCity class and renderCity have been removed.
  // The new 3D city class and renderCity are defined above (outside DOMContentLoaded).
  // The following functions (messages, bosses, etc.) remain unchanged.
